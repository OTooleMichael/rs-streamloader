...
function RedshiftLoader(options)
{
	...
	*/
	...
	options = mergeOptionsAndDefaults(options,this.defaults.config);
	...
	let bodies = (options.body) ? [options.body] : options.bodies || [];
	bodies.forEach((b)=>this.addFile(b));
...
RedshiftLoader.prototype.addFile = function(uploadBody)
{
	...
	task.i = i;
	task.key = `${filePrefix}_${this.jobTime}_prt_${i}.txt`;
...
RedshiftLoader.prototype._start = function(uploadType,cb) {
	...
	.then(()=>transactionQuery(this.rsPool, this.makeQueries()) )
	.then(()=>{
		this.emit('progress',{
			task:'done',
			res:"SUCCESS"
...
RedshiftLoader.prototype.uploadManifest = function(){
    ...
    this.manifestKey = `${filePrefix}_${this.jobTime}_manifiest.json`;
...
RedshiftLoader.prototype.cancelUploads = function() {
	this.bodies.forEach(function(task){
		if(!task.uploaded) task.managedUpload.abort();
		return
	});
	return this
}
...
RedshiftLoader.prototype._wrapUp = function(error) {
	...
	if(s3Cleanup == 'ALWAYS' || s3Cleanup === true || (!error && s3Cleanup == "SUCCESS" ) )
	{
		this.cleanUpS3();
	};
	if(typeof this.rsPool.end == 'function') this.rsPool.end();
...
RedshiftLoader.prototype.makeQueries = function()
{
    ...
    {
    	let DROP_TABLE = (removeTempTable) ? `DROP TABLE ${TEMP_TABLE};` : ''
    	return [`
		  BEGIN TRANSACTION;
		...
		`
		  DELETE FROM ${TABLE}
		  WHERE ${idField} IN (SELECT DISTINCT ${idField} FROM ${TEMP_TABLE} );
		  INSERT INTO ${TABLE}
