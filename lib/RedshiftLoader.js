...
RedshiftLoader.prototype.getQualifiedTable = function(tableType='table') {
	if(tableType === 'table'){
		const {schema,table} = this.options[tableType];
    	return schema ? schema+'.'+table : table;
	}
	if(tableType !== 'loadingTable'){
		throw new Error('Invalid tableType: '+tableType)
	}
	const {table,loadingTable} = this.options
	if(loadingTable == undefined){
		return this.getQualifiedTable('table')
	}
	return loadingTable.schema+'.'+( loadingTable.table||table.table );
}
RedshiftLoader.prototype.getFilePrefix = function() {
	const {filePrefix} = this.options
...
RedshiftLoader.prototype._start = function(uploadType,cb) {
	...
	.then(()=>{
		...
		return transactionQuery(this.rsPool,q,cleanUp) 
	})
	.then(()=>{
		this._wrapUp();
		this.emit('progress',{
...
RedshiftLoader.prototype._wrapUp = function(error) {
	...
	const isAlways = (s3Cleanup == 'ALWAYS' || s3Cleanup === true)
