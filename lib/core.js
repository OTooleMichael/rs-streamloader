...
function transactionQuery(rsPool,queries){
	...
	.catch(function(err)
	{
		...
		if(typeof message == 'string' && message.search("'stl_load_errors'") !== -1)
		{
			return queryClient(clientObject,'ROLLBACK;')
			.then(()=> queryClient(clientObject,'SELECT * FROM stl_load_errors ORDER BY starttime DESC LIMIT 1'))
			.then(function(rows)
			{
