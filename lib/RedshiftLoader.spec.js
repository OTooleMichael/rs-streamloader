...
const { AWS_CREDS, AWS_BUCKET, REDSHIFT_DETAILS} = CONFIG
...
describe('Redshift Loader ', function() {
	...
	it('Gets Quailified Tables',function() {
		...
		expect(ld.getQualifiedTable()).to.equal('schema1.rs_loader') // defaults to 'table'
		expect(ld.getQualifiedTable('table')).to.equal('schema1.rs_loader')
		expect(ld.getQualifiedTable('loadingTable')).to.equal('schema1.rs_loader')
		params = Object.assign({}, options, {
			table:{table:'rs_loader',schema:'schema1'},
			loadingTable:{table:'rs_loader_temp',schema:'loading'}
		})
		ld = new RedshiftLoader(params)
		expect(ld.getQualifiedTable('loadingTable')).to.equal('loading.rs_loader_temp')
		params = Object.assign({}, options, {
			table:'schema1.rs_loader',
			loadingTable:{schema:'loader'}
		})
		ld = new RedshiftLoader(params)
		expect(ld.getQualifiedTable('loadingTable')).to.equal('loader.rs_loader')
	})
	it('Gets Quailified Tables only accepts certain params',function() {
		...
		let ld = new RedshiftLoader(params)
		expect(ld.getQualifiedTable()).to.equal('schema1.rs_loader')
		ld = new RedshiftLoader(params)
		expect(()=>ld.getQualifiedTable('LEMON')).to.throw('Invalid tableType')
	})
	it('Accepts a  or string as filePrefix',function(){
		let params = Object.assign({}, options, {
			filePrefix:(rs)=>'/'+rs.getQualifiedTable('table').replace('.','/'),
			table:{table:'rs_loader',schema:'schema1'},
		})
		let ld = new RedshiftLoader(params)
		expect(ld.getFilePrefix()).to.equal('/schema1/rs_loader')
		params = Object.assign({}, options, {
			filePrefix:'lemon/',
			table:{table:'rs_loader',schema:'schema1'},
		})
		ld = new RedshiftLoader(params)
		expect(ld.getFilePrefix()).to.equal('lemon/')
	})
});
